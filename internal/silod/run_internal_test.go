...
func installFakes(t *testing.T, httpSub, grpcSub, bootSub, gossipSubFake *fakeSubsystem) {
	...
	newGRPCSubsystem = func(_ *config.Config, _ *tls.Config, _ *transport.TokenAuthenticator, _ chunkstore.Store, _ transport.Coordinator, _ transport.NamespaceOps, _ transport.StatusMembers, _ transport.Drainer, _ string, _ *slog.Logger) subsystem {
		return grpcSub
	}
