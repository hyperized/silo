...
| M10 | in progress | production hardening: pluggable key provider (file fixed) + cloud KMS (AWS/GCP/Azure), node-cert auto-rotation, threat model, backup export to S3/GCS/Azure Blob, CRL-based revocation (`siloctl ca revoke` + `SILO_TLS_CRL`) done; rolling-upgrade version handshake, signed CSI/FUSE tokens, runbook remaining |
