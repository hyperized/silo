...
What is *not* finished, why, and what it would take to close. Kept current as
the work lands.
...
- `make bench` runs the in-process micro-benchmarks: AES-GCM encrypt/decrypt, the
...
- `make bench-cluster` runs end-to-end benchmarks against a real 3-node silod
...
| `WriterSDK` (4 MiB stream, multi-chunk fan-out) | n/a | n/a | 25 ms / 166 MB/s |
| `ReaderSDK` (4 MiB stream, manifest replay) | n/a | n/a | 5.0 ms / 847 MB/s |
| `Stat` (metadata RPC) | 76 µs | n/a | n/a |
| `NamespaceMkdir` (CRDT entry) | 1.0 ms | n/a | n/a |
...
- **fsync-per-`Put`** (file + dir) is synchronous on the write path, which is
  correct for crash consistency. Batching multiple chunk writes behind one fsync would raise
