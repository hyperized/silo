...
- **Observability listeners are unauthenticated HTTP.** silod's
  `SILO_HTTP_ADDR` (metrics, health, optional pprof) and the CSI node
  plugin's `SILO_CSI_HTTP_ADDR` (metrics, health) expose operational
  readings, never data or credentials. Keep them on the node or cluster
  network anyway.
...
silo authenticates with mTLS (identity) and, when `SILO_REQUIRE_TOKENS=1`,
authorizes client-cert callers with **signed capability tokens** (permitted
operations). A token is an Ed25519-signed assertion: a `principal`, a set of
capabilities (`chunk:read`, `chunk:write`, `namespace:read/write`, `status:read`,
`node:admin`, or `*`), and an expiry. It is minted offline with the cluster CA key
...
(certs with a `spiffe://silo/node/` identity) are exempt, because peer-to-peer
replication is trusted by membership. Only external clients (CSI/FUSE/
